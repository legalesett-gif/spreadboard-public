...
#: restic's default `--max-unused 5%` forces aggressive repacking. Applying the
#: retention policy to a backlog for the first time queued 27.5 GiB of repack,
#: which ran at ~38 KB/s against Google Drive's request quota -- about nine days,
#: all of it holding the repository's EXCLUSIVE lock and so blocking every later
#: backup and restore. Tolerating waste is far cheaper than a stalled backup
#: (Drive had 21 GiB free of 100 GiB), and a bounded repack keeps each run inside
#: one timer window so retention progresses steadily instead of never finishing.
PRUNE_MAX_UNUSED = os.environ.get("SPREADBOARD_BACKUP_MAX_UNUSED", "30%")
PRUNE_MAX_REPACK = os.environ.get("SPREADBOARD_BACKUP_MAX_REPACK", "64M")
