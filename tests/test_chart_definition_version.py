...
@pytest.mark.parametrize('revision,refresh_first', [(None, True), (1, True), (2, False)])
...
def test_refresh_marks_new_definitions_only_when_required_sources_succeeded(tmp_path, monkeypatch, failed):
    ...
    venues = {'Binance': 'binance', 'Bitget': 'bitget', 'Bybit': 'bybit', 'Hyperliquid': 'hyperliquid'}
    monkeypatch.setattr(chart_catalog, 'VENUE_IDS', venues)
    monkeypatch.setattr(chart_catalog, 'NATIVE_FUTURES_VENUES', set(venues))
    monkeypatch.setattr(chart_catalog, 'NATIVE_SPOT_VENUES', set())
    ...
    def job(venue, market_type):
        if failed and venue == 'Hyperliquid':
    ...
    assert result.get('definition_revision') == (1 if failed else 2)
