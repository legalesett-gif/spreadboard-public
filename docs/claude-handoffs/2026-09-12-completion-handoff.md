...
## Later Codex release — supersedes older pending venue/history sections

Source `61448a7` deployed to app and collector on September12, preserving Claude backup fixes and review commits. Shared branch fast-forwarded to this tested source. Full suite2,949 passed; Ruff0 new (500 baseline); both service digests324beb954b108ebf match, healthy with zero restart counters/OOM. No private exchange action or Telegram send.

Implemented Phemex/HTX/Huobi public collector/read exclusion (historical/private records retained), bounded work-conserving settlement scheduling, collision-safe English Lobster search (`龙虾` identity unchanged), and exhaustive native Aster mixed-cadence history validation. Production canonical API: Futures237 rows across14 venues/no retired venues in sampled25 tokens (2.14s); ESPORTS30d42 rows/best gross10.4281% (1.01s); ANSEM7d20 rows/best gross3.78830819% (3.10s); SIREN7d1.975%; Lobster30d11.023827%. These are dated samples, not guarantees. Hyperliquid ANSEM has only~18 days verified history, so that leg's30d remains unavailable; other ANSEM venues may have longer history.

Remaining scope: reproduce user's exact narrow Futures URL/filter, audit legacy historical kind/unsupported-filter fallback (canonical Funding queries passed), and measure broad freshness/completeness soak. Reverse long-futures/short-spot gross funding can be mathematically correct while requiring borrow/inventory and excluding costs. Await user preference before changing default direction. Do not claim all ranked rows executable or all venues/history fully covered.
...
Updated user policy on September 12: remove Phemex AND HTX from public discovery, quote sweeps, opportunity subscriptions and displayed opportunities, not merely funding. Preserve historical/private accounting records. Ourbit remains publicly excluded; CoinEx remains funding-excluded with price coverage retained. Codex is implementing this and funding-refresh/search corrections in `tmp/spreadboard-venue-retirement`, branch `codex/retire-phemex-htx-20260912`. Do not overwrite that release with the old policy. Coordinate production deployment and consult REMINDERS for status. Follow latest implemented DEX taxonomy and user policy, inspect differences with older OKX-only requests before changing it. No silent venue substitutions. Spot-Spot and Spot-DEX public lanes were retired by request; do not restore them casually.

New funding audit requirement: investigate missing/inaccurate 7d/30d leaders across all venues, using ESPORTS, ANSEM, SIREN and LOBSTER as regression examples, not hardcoded winners. Fresh September 12 evidence found refresh-expiry gaps, mixed hourly/four-hour Aster history rejected by a single-cadence validator, and LOBSTER listed under `龙虾`. Several extreme leaders are reverse futures-long/spot-short routes requiring inventory/borrow; do not call gross carry an executable net return. Validate both sides, exact historical interval, fees/borrow distinction and initial/streamed ranking. A complete source check is not complete rolling-window coverage.
