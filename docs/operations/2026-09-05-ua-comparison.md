...
## SpreadBoard stability and UA comparison — Codex continuation (2026-09-06, 01:44 UTC)

- Goal ACTIVE. Release 6eb1788 is LIVE app/collector, source digest 1c9fe38134877f66, started 2026-09-06T01:40:34Z. Guard clear before deploy and immediately before recreation. Deploy exit0, health200, both digest matches; fresh01:44 both healthy/restarts0/OOMfalse. Accounting untouched. Heartbeat freshly PAUSED; finite observer PID364378 still active; do not duplicate.
- New changes: exact advanced metadata/volume/persistence filters reuse broad policy; known exact empty result does not fall back; full matching funding counts and headline construction; advertised 24h volume sorts on thinner leg volume instead of matched probe depth; navigation Store.route_index now uses checksum-verified streaming reader rather than full JSON buffer.
- Validation: final full suite 2655 passed159.87s; Ruff517known/no new; streaming tests23passed. Old-source advanced-filter mutant9failed and volume/stream mutants2failed. First full run failed existing volume label requirement; restored label and corrected sort metric, rather than weakening the test. Test data preserved individually under output/stability-20260906/test-data-preserved-final and restored fromHEAD.
- Actual authenticated UI AFTER deployment: OPENAI35routes and27funding pairs; exact headings corrected. Extreme min_volume_24h_usd=1000000000000000 returns0assets/0routes/0funding and no broad-cache fallback. Initial navigation timed out at CUA during startup but subsequently completedHTTP200. Another filtered navigation exceededCUA10s then completedHTTP200. Full-page latency remains open.
- NEW LIVE FINDING: Top Funding Pairs sidebar still says no rows despite27funding pairs. The helper-level test passes, so trace final materialized_views.finalize_projection plus _apply_spread_freshness_coalesced through _sync_telegram_client_universe. Do not claim sidebar fixed; add end-to-end regression at this call site. Also verify refreshed Hyperliquid coverage after startup: initial postdeploy OPENAI exchange menu lackedHyperliquid whereas predeploy includedit; freshness may explain, but current presence is unresolved until fresh checked.
- Memory: prior010882d appVmHWM3679560KiB and cgrouppeak3584MiB(cap), collector sampledmaxanon2840MiB with materializer2078MiB/parent153MiB. New streaming fallback addresses allocation mechanism; NO measured production savings yet. Need ordinary navigation rotation and CPU/anon/cgroup measurements. Do not lower caps on inference.
- Evidence: exact-filter-final-pytest.txt, exact-filter-final-ruff.txt, exact-filter-mutant.txt, volume-stream-mutant.txt, exact-filter-deploy.txt. New source/test commit6eb1788. No source edits since deploy. Retain prior comparator/history/backup evidence below; these are timestamped samples, not exhaustive parity.
