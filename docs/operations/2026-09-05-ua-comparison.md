...
## SpreadBoard stability and UA comparison — Codex continuation (2026-09-06, 00:42 UTC)

- Goal ACTIVE. Previous turn made verified progress: native oracle and BingX public-status fixes deployed, live UI and comparator evidence refreshed. Stability heartbeat stays PAUSED; existing finite read-only observer remains active until00:57:52UTC. No duplicate observer, trades, messages, spend, Pushover, cap increases or weakened truth gates.
- Own checkout `tmp/spreadboard-collector-retention`, branch `codex/collector-retention-20260906`. Other accounting work in `tmp/spreadboard-funding-publication` (c115414) and dirty `tmp/spreadboard-funding-current-truth` untouched. Accounting container runs its separate release.
- LIVE app/collector **aef8e24 / 8eeed9c54dbefd51**, started **00:38:02 UTC Sep6**. Deploy exit0, health200, exact digest parity; fresh healthy/restarts0/OOMfalse. App9c8baeca4da44b74b8049789d7fcfc607a024a6b931336780b8706816fb613ac; collector765191aaefc90b5b90471e4a665014aee8bc9a2fd9cb895a4ca4ab2616f72b06. Both discovery AND finalizer absent before deployment and immediately before recreation, no force.
- Final gate **2630 passed in138.65s**, Ruff517 known/zero new. CAP tests fail old source3/3. Oracle gate2626 passed114.75s; old oracle source6failed2passed. Test-mutated8tracked data files preserved separately under output and restored individually from HEAD; no source edits remain pending.
- Delivered03b6e97: keep KuCoin/Phemex native index fields; supplement MEXC/HTX/Bitget funding-only feeds with exact-ID all-contract oracle joins. Reject nonfinite/nonpositive, stale>90s or future>1s supplemental indexes. No mark/last-price substitution; failures retain actual funding and log a diagnostic. Additional bulk calls only for missing indexes. Raw fresh feeds00:15 and candidate probe00:21 proved all five.
- Live oracle proof after ordinary rotation at00:31: all9OPENAI GateSpot~867 versus perpetual~1400 combinations now have native indexes and are excluded by the existing spot/perp guard. Authenticated Markets OPENAI page showed17futures-only alternatives, no expanded spot pairs, and intact Hyperliquid `io:OAI` links. First navigation exceeded tool wait; later warm page loaded in2.685s tool wall time. Do not equate that with TTFB or cold-latency acceptance.
- Deliveredaef8e24: BingX CAP native status1 with apiStateOpen/Close=false was incorrectly treated as a closed public market. Public copied definitions now follow native status while original CCXT client metadata stays active=false; API order permissions still require private preflight. Status25 and unknown/inactive definitions remain excluded. Catalogue, broad discovery and current funding share that rule. Fresh00:41: CAP funding now published(-.1269%/4h,index.04477), but the catalogue remains its23:56generation without CAP. Wait for ordinary hourly catalogue refresh (roughly00:56) and verify complete catalogue + exact published Funding pair; do not claim the visible CAP gap closed yet.
- Latest UA sample~00:25:15exactvisible futures pairs.12non-Ourbit pairs present in fresh catalogue-pair construction and complete funding cache; two deliberate Ourbit exclusions; CAP BingX gap diagnosed above. HOOD WhiteBIT->Bybit is in complete funding/fresh pair lookup but absent from the structural spread index; negative basis explains why index-only coverage is insufficient. ANSEM Aster->Hyperliquid now present. All12 matched pairs funding-eligible, APR/365 normalizes UA to projected/day; several rates moved between observations, so differences are not automatically errors. No claim of simultaneous full equality or exhaustive premium parity. Prior15/59samples overlap; never sum them.
- NEW UI correctness follow-up: exact OPENAI search returns populated futures group but route/asset tabs report0; pagination says37assets though it slices37routes of one token and has no has_more/has_previous fields, so Next stays disabled. Root is `server._exact_catalog_market_projection` (~2954): empty kind/asset count maps, route-based pagination lacking flags. `render_market_pagination` (~10090) hardcodes assets. Fix counts, units and access to all matching exact routes; preserve real filters and avoid counting only first25routes. Evidence `openai-ui-verification.json`.
- Collector retention and funding-reader fixes delivered earlier remain: parent~150–160MB,index_rows0. Source095a686 ordinary materializer successfully published17views/197321routes in318.276s child/333.3s total, versus prior~15min abort. Parent stayed~150MB afterward. Live-index heavy worker still reached~2357MBRSS/cgroup3704MiB on prior segment; lower caps remain unproved. Caps3584/4096/768/192MiB unchanged.
- BACKUP NOT ACCEPTED: scheduled job started00:19:49UTC, still activating/MainPID336267 at00:39; children restic337325 in backup phase and rclone337334. Journal classification shows8Google Drive rateLimitExceeded/403 responses,40error mentions, no snapshot saved. Over124.5s rclone IO advanced only~592kB; restic waited. Unit TimeoutStartUSec=infinity. No process interrupted, no config/retention changed, no manual backup/prune/check. Prior18:20job failed; backend read-only snapshots23:51 had succeeded but did not prove writes. Next action: diagnose rate-limited upload/backoff safely, preserve secrets, avoid concurrent repo mutations. Do not call current activation a success.
- Existing observer `spreadboard-stability-native-status-20260905.service` remains active; source segment must start00:38:02. Its5min endpoint cadence cannot prove final-release30samples/hour. Still open: CAP ordinary publication, exact-token counters/pagination, backup successful timer/prune/check, cold navigation latency, final-release30-sample/hour±10%, safe lower caps and clean48h. No completion claim.
- Evidence under `output/stability-20260906/`: oracle-final-full-pytest.txt, oracle-ruff.txt, oracle-mutant.txt, oracle-deploy.txt, native-oracles.jsonl, candidate-oracle-check.jsonl, openai-indexes-rotated.json, openai-ui-verification.json, ua-funding-0025.json, ua-reconciliation-0025.jsonl, public-status-focused.txt, public-status-full-pytest.txt, public-status-ruff.txt, public-status-mutant.txt, public-status-deploy.txt, backup-progress-a/b.json. Latest cardinality00:05:1987futures/3962spot token labels,9733/12484markets, zero duplicate exact keys; user-reportedUA2534/4370 counts are token labels, not directed routes.
