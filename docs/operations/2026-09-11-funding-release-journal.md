...
## 06:22 UTC — hourly recovery complete; first normal backup running

Fresh public health06:22:47 reports zero overdue1d/7d/30d windows, zero retryable errors and current_window_catch_up_complete=true. Normal06:00expiry/catch-up completed withoutmanual repair; separate deep-historypending25(from197). The first normal backup timer run started06:21:17UTC,PID2555410. At06:22 it remainedactivating/start with no exit timestamp; neither priorResult=success nor interimExecMainStatus=0 proves completion. Observer2538662 remainsactive. Evidence: `hourly-rollover-recovery.json`, `first-normal-backup-start.json`. No production source/cap changes.
