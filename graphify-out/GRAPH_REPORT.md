...
## Corpus Check
- 70 files · ~152,587 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2848 nodes · 7073 edges · 45 communities detected
...
- `test_fast_quote_selection_keeps_top_tokens_and_their_other_routes()` --calls--> `_expanded_token_rows()`  [INFERRED]
  tests/test_live_charts.py → spreadboard/fast_quotes.py
- `test_fast_quote_gate_client_uses_available_ccxt_alias()` --calls--> `FastQuoteRefresher`  [INFERRED]
  tests/test_live_charts.py → spreadboard/fast_quotes.py
- `test_native_gate_spot_order_book_is_sorted_and_normalized()` --calls--> `_native_spot_order_book()`  [INFERRED]
  tests/test_live_charts.py → spreadboard/fast_quotes.py
- `test_secondary_native_futures_books_are_normalized()` --calls--> `_native_order_book()`  [INFERRED]
  tests/test_live_charts.py → spreadboard/fast_quotes.py
...
Cohesion: 0.01
Nodes (47): _(), an(), ar(), as(), bi(), ct, ds(), fi (+39 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (307): active_class(), api_alert_context(), api_alert_preview(), api_board(), api_community(), api_funding_watch(), api_health(), api_history() (+299 more)

### Community 2 - "Community 2"
Cohesion: 0.01
Nodes (64): At, b(), bn(), Bt, c(), cn(), d(), De() (+56 more)

### Community 3 - "Community 3"
Cohesion: 0.02
Nodes (196): _checks_from_payload(), empty(), ExecutorAttestation, ExecutorAttestationRegistry, load_executor_attestations(), Read-only executor/preflight attestations for API discovery rows., route_key(), BlacklistFilterResult (+188 more)

### Community 4 - "Community 4"
Cohesion: 0.03
Nodes (20): a(), ae, ai, ci, ge, gt, ie, je() (+12 more)
...
Cohesion: 0.03
Nodes (12): bs(), er, es(), is(), js(), rr(), se, ss (+4 more)
...
Cohesion: 0.04
Nodes (9): ft, ht, In(), lr, nt, ot, tn(), Wt() (+1 more)

### Community 10 - "Community 10"
Cohesion: 0.06
Nodes (35): BaseHTTPRequestHandler, _atomic_write_snapshot(), _env_bool(), _funding_lane(), _funding_refresh_route_keys(), _log(), main(), _merge_newer_fast_quotes() (+27 more)

### Community 11 - "Community 11"
Cohesion: 0.07
Nodes (66): _age_min(), _bool_or_none(), _dedupe_rows(), _depth_from_api(), _dex_contract_mirage_reasons(), _dex_spot_source_status(), _effective_funding_24h(), _effective_funding_24h_dict() (+58 more)

### Community 12 - "Community 12"
Cohesion: 0.05
Nodes (58): _allocate_amount(), config(), create_invoice(), CryptoBillingError, CryptoConfig, format_amount(), get_invoice(), _invoice_dict() (+50 more)
...
Cohesion: 0.04
Nodes (7): ce, hr, ir, Ks, rt, tr, we

### Community 16 - "Community 16"
Cohesion: 0.05
Nodes (30): test_settled_funding_propagates_to_every_route_using_same_leg(), board_event(), board_file(), message(), Token lookups in the subscriber Telegram group., Whatever a member types becomes a bounded, inert token string., Whatever a member types becomes a bounded, inert token string., Whatever a member types becomes a bounded, inert token string. (+22 more)
...
Cohesion: 0.12
Nodes (23): _api_call(), config(), _configure_group(), _create_join_request_link(), _handle_group_query(), _handle_join_request(), handle_update(), link_url() (+15 more)
...
## Knowledge Gaps
- **234 isolated node(s):** `HTTP boundary for crypto checkout.  The subtle failure this guards against: memb`, `Crypto (Arbitrum USDC/USDT) prepaid billing.  The invariant that matters most: a`, `Dollars -> raw 6-decimal token units.`, `An exchange withdrawal fee must not cost the member their access.`, `A token calling itself USDC must not buy access.` (+229 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 34`** (7 nodes): `be`, `.CM()`, `.constructor()`, `.dM()`, `.draw()`, `.et()`, `.ht()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 40`** (3 nodes): `main()`, `configure_telegram_webhook.py`, `telegram_call()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 41`** (3 nodes): `main()`, `configure_stripe_webhook.py`, `stripe_request()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 44`** (2 nodes): `Spread arbitrage research bot.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 45`** (2 nodes): `__init__.py`, `Utility helpers for spreadarb.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (2 nodes): `Narrow live-operation helpers.  The main live engine remains locked. Modules in`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 47`** (2 nodes): `Read-only decentralized exchange integrations.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 48`** (2 nodes): `Read-only API discovery for Telegram opportunity visibility.  The package is int`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `FastQuoteRefresher` connect `Community 5` to `Community 1`, `Community 10`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `SpreadBoardServer` connect `Community 10` to `Community 1`, `Community 5`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `_release_ccxt_exchange()` connect `Community 3` to `Community 5`?**
