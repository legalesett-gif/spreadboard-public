...
## Corpus Check
- 70 files · ~156,640 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2929 nodes · 7181 edges · 50 communities detected
...
Cohesion: 0.01
Nodes (40): _(), bn(), c(), ct, ds(), e(), En, fi (+32 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (79): a(), ae, ai, as(), At, bi(), bs(), Bt (+71 more)
...
Cohesion: 0.02
Nodes (193): _checks_from_payload(), empty(), ExecutorAttestation, ExecutorAttestationRegistry, load_executor_attestations(), Read-only executor/preflight attestations for API discovery rows., route_key(), BlacklistFilterResult (+185 more)
...
Cohesion: 0.03
Nodes (7): ar(), be, ee(), lt, ve, vt, yi

### Community 6 - "Community 6"
Cohesion: 0.05
Nodes (81): _age_min(), _bool_or_none(), _dedupe_rows(), _depth_from_api(), _dex_spot_source_status(), _effective_funding_24h(), _effective_funding_24h_dict(), _entrance_spread() (+73 more)
...
## Knowledge Gaps
- **284 isolated node(s):** `HTTP boundary for crypto checkout.  The subtle failure this guards against: memb`, `The tutorial must be reachable without an account -- it is a conversion page.`, `Crypto (Arbitrum USDC/USDT) prepaid billing.  The invariant that matters most: a`, `Dollars -> raw 6-decimal token units.`, `An exchange withdrawal fee must not cost the member their access.` (+279 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 23`** (23 nodes): `ce`, `.applyOptions()`, `.constructor()`, `.detach()`, `.getPane()`, `.mM()`, `.attachPrimitive()`, `.detachPrimitive()`, `.ca()`, `.detached()`, `.Tt()`, `.detachPrimitive()`, `we`, `.constructor()`, `.detach()`, `.et()`, `.getSeries()`, `.ht()`, `.jp()`, `._M()`, `.mM()`, `.oM()`, `.uM()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 26`** (17 nodes): `.setData()`, `js()`, `.cacheKey()`, `.constructor()`, `.convertHorzItemToInternal()`, `.createConverterToInternalObj()`, `.fillWeightsForPoints()`, `.formatHorzItem()`, `.formatTickmark()`, `.key()`, `.maxTickMarkWeight()`, `.options()`, `.preprocessData()`, `.setOptions()`, `.updateFormatter()`, `.zg()`, `.eg()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 32`** (12 nodes): `di()`, `o`, `.constructor()`, `.hi()`, `.ht()`, `.i()`, `.m()`, `.p()`, `.st()`, `.$t()`, `.u()`, `.v()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 33`** (12 nodes): `it`, `.constructor()`, `.ih()`, `.Ns()`, `.st()`, `.st()`, `pt`, `.constructor()`, `.ih()`, `.st()`, `.kt()`, `.draw()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 45`** (3 nodes): `main()`, `configure_telegram_webhook.py`, `telegram_call()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (3 nodes): `main()`, `configure_stripe_webhook.py`, `stripe_request()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 49`** (2 nodes): `Spread arbitrage research bot.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 50`** (2 nodes): `__init__.py`, `Utility helpers for spreadarb.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 51`** (2 nodes): `Narrow live-operation helpers.  The main live engine remains locked. Modules in`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 52`** (2 nodes): `Read-only decentralized exchange integrations.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 53`** (2 nodes): `Read-only API discovery for Telegram opportunity visibility.  The package is int`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `FastQuoteRefresher` connect `Community 4` to `Community 9`, `Community 2`?**
  _High betweenness centrality (0.069) - this node is a cross-community bridge._
- **Why does `WatchAsset` connect `Community 3` to `Community 11`, `Community 6`?**
  _High betweenness centrality (0.042) - this node is a cross-community bridge._
- **Why does `SpreadTerminalRow` connect `Community 6` to `Community 3`?**
