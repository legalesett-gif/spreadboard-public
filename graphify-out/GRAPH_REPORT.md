...
## Corpus Check
- 111 files · ~216,416 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5522 nodes · 10513 edges · 84 communities detected
...
Cohesion: 0.01
Nodes (116): _(), a(), ae, an(), ar(), as(), At, b() (+108 more)
...
Cohesion: 0.02
Nodes (367): _daily_pct(), main(), A venue's per-interval rate as percent per day., A venue's per-interval rate as percent per day., _exchange(), main(), A loaded CCXT client for a venue id, or None if it has no adapter., active_class() (+359 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (266): _checks_from_payload(), empty(), ExecutorAttestation, ExecutorAttestationRegistry, load_executor_attestations(), Read-only executor/preflight attestations for API discovery rows., route_key(), BlacklistFilterResult (+258 more)

### Community 5 - "Community 5"
Cohesion: 0.02
Nodes (21): ai, bs(), ci, ct, er, es(), ge, gt (+13 more)
...
Cohesion: 0.03
Nodes (80): _allocate_amount(), config(), create_invoice(), CryptoBillingError, CryptoConfig, format_amount(), get_invoice(), _invoice_dict() (+72 more)

### Community 14 - "Community 14"
Cohesion: 0.02
Nodes (88): _health_from_snapshot_file(), _legacy_board_snapshot(), _load_public_intel_feed(), _market_cache_stale_get(), _market_stale_key(), _public_intel_best_route(), _public_intel_payload(), _public_intel_route_reality() (+80 more)
...
Cohesion: 0.04
Nodes (50): api_alert_context(), _market_warming_payload(), Return only the local market rows needed to evaluate browser-local rules., Return only the local market rows needed to evaluate browser-local rules., Return only the local market rows needed to evaluate browser-local rules., Return only the local market rows needed to evaluate browser-local rules., Return only the local market rows needed to evaluate browser-local rules., Return only the local market rows needed to evaluate browser-local rules. (+42 more)
...
Cohesion: 0.05
Nodes (37): A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre (+29 more)
...
Cohesion: 0.12
Nodes (28): _align(), _atomic_json(), _cache_path(), _claim_warming(), evenly_sample(), _fetch_and_cache(), _fetch_leg(), _fetch_ourbit_leg() (+20 more)
...
Cohesion: 0.08
Nodes (24): Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you (+16 more)
...
Cohesion: 0.14
Nodes (19): _any_network_state(), _atomic_write(), _bool_or_none(), _fetch_native_venue_rails(), _fetch_venue_rails(), _load_payload(), _normalize_network(), _payload_is_fresh() (+11 more)
...
Cohesion: 0.28
Nodes (15): _append_missing_broad_source_errors(), _broad_dex_command(), _broad_dex_error_payload(), _broad_dex_error_scan(), _broad_dex_partial_or_error_payload(), _broad_dex_sources(), build_parser(), main() (+7 more)

### Community 46 - "Community 46"
Cohesion: 0.12
Nodes (16): A member's own pinned pairs, and the control to pin the one on screen.      Any, A member's own pinned pairs, and the control to pin the one on screen.      Any, A member's own pinned pairs, and the control to pin the one on screen.      Any, A member's own pinned pairs, and the control to pin the one on screen.      Any, A member's own pinned pairs, and the control to pin the one on screen.      Any, A member's own pinned pairs, and the control to pin the one on screen.      Any, A member's own pinned pairs, and the control to pin the one on screen.      Any, A member's own pinned pairs, and the control to pin the one on screen.      Any (+8 more)
...
Cohesion: 0.17
Nodes (9): _clear_cache(), Concurrent cold builds of *different* views must not stack up.  Single-flight al, A view built before must answer from the previous copy under load., It must not add a second long wait on top of the in-flight gate.      A request, Both stores, or one test is served the previous test's stale payload., A leaked slot would wedge the board permanently., test_a_queued_reader_is_served_rather_than_left_waiting(), test_admission_control_fails_fast() (+1 more)
...
## Knowledge Gaps
- **2083 isolated node(s):** `HTTP boundary for crypto checkout.  The subtle failure this guards against: memb`, `The tutorial must be reachable without an account -- it is a conversion page.`, `The crypto payment path, end to end.  The watcher is what makes a crypto payment`, `It was only started by server.py's CLI main(), which production does not run.`, `A cold cursor must not scan Arbitrum from block 0.` (+2078 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 47`** (16 nodes): `js()`, `.cacheKey()`, `.constructor()`, `.convertHorzItemToInternal()`, `.createConverterToInternalObj()`, `.fillWeightsForPoints()`, `.formatHorzItem()`, `.formatTickmark()`, `.key()`, `.maxTickMarkWeight()`, `.options()`, `.preprocessData()`, `.setOptions()`, `.updateFormatter()`, `.zg()`, `.eg()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 50`** (15 nodes): `.Wg()`, `.Fg()`, `le`, `.constructor()`, `.Ig()`, `ne()`, `.attached()`, `.constructor()`, `.detached()`, `.hr()`, `.NM()`, `.paneViews()`, `.SM()`, `.updateAllViews()`, `.Wg()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 60`** (12 nodes): `di()`, `o`, `.constructor()`, `.hi()`, `.ht()`, `.i()`, `.m()`, `.p()`, `.st()`, `.$t()`, `.u()`, `.v()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Thin community `Community 76`** (4 nodes): `f()`, `.constructor()`, `.ht()`, `.st()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 78`** (3 nodes): `main()`, `configure_telegram_webhook.py`, `telegram_call()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 79`** (3 nodes): `main()`, `configure_stripe_webhook.py`, `stripe_request()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 80`** (3 nodes): `qt`, `.constructor()`, `.lr()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 85`** (2 nodes): `Spread arbitrage research bot.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 86`** (2 nodes): `__init__.py`, `Utility helpers for spreadarb.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 87`** (2 nodes): `Narrow live-operation helpers.  The main live engine remains locked. Modules in`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 88`** (2 nodes): `Read-only decentralized exchange integrations.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (2 nodes): `Read-only API discovery for Telegram opportunity visibility.  The package is int`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `q()` connect `Community 0` to `Community 18`, `Community 2`?**
  _High betweenness centrality (0.279) - this node is a cross-community bridge._
- **Why does `_Book` connect `Community 4` to `Community 1`, `Community 2`, `Community 6`, `Community 7`?**
  _High betweenness centrality (0.256) - this node is a cross-community bridge._
- **Why does `FastQuoteRefresher` connect `Community 6` to `Community 3`, `Community 4`, `Community 7`, `Community 40`, `Community 30`, `Community 31`?**
