...
## Corpus Check
- 70 files · ~156,581 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2924 nodes · 7175 edges · 54 communities detected
...
Cohesion: 0.01
Nodes (82): a(), ae, ai, as(), At, b(), bi(), bs() (+74 more)

### Community 1 - "Community 1"
Cohesion: 0.01
Nodes (40): _(), an(), ar(), bn(), ds(), e(), En, fi (+32 more)

### Community 2 - "Community 2"
Cohesion: 0.02
Nodes (313): active_class(), api_alert_context(), api_alert_preview(), api_board(), api_community(), api_funding_watch(), api_health(), api_history() (+305 more)
...
Cohesion: 0.04
Nodes (86): depth_weighted_price(), Small read-only order-book math helpers., _audit_route(), _endpoint(), _formula_errors(), _funding_errors(), _history(), _json_url() (+78 more)

### Community 5 - "Community 5"
Cohesion: 0.05
Nodes (79): _age_min(), _bool_or_none(), _dedupe_rows(), _depth_from_api(), _dex_contract_mirage_reasons(), _dex_spot_source_status(), _effective_funding_24h(), _effective_funding_24h_dict() (+71 more)
...
Cohesion: 0.04
Nodes (9): es(), is(), js(), rr(), se, ss, tt, xn() (+1 more)
...
Cohesion: 0.06
Nodes (38): _atomic_write_snapshot(), _env_bool(), _funding_lane(), _funding_refresh_route_keys(), _log(), main(), _merge_newer_fast_quotes(), _rank_value() (+30 more)
...
Cohesion: 0.26
Nodes (2): BaseHTTPRequestHandler, SpreadBoardHandler
...
Cohesion: 0.29
Nodes (7): _merge_history_rows(), Merge proxy and exact samples without sacrificing full-window coverage., Merge proxy and exact samples without sacrificing full-window coverage., Merge proxy and exact samples without sacrificing full-window coverage., Merge proxy and exact samples without sacrificing full-window coverage., Merge proxy and exact samples without sacrificing full-window coverage., Merge proxy and exact samples without sacrificing full-window coverage.
...
## Knowledge Gaps
- **280 isolated node(s):** `HTTP boundary for crypto checkout.  The subtle failure this guards against: memb`, `The tutorial must be reachable without an account -- it is a conversion page.`, `Crypto (Arbitrum USDC/USDT) prepaid billing.  The invariant that matters most: a`, `Dollars -> raw 6-decimal token units.`, `An exchange withdrawal fee must not cost the member their access.` (+275 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 22`** (19 nodes): `BaseHTTPRequestHandler`, `SpreadBoardHandler`, `._authorize()`, `.do_HEAD()`, `.do_POST()`, `._handle_login()`, `._handle_register()`, `.log_message()`, `._read_payload()`, `._read_raw_body()`, `._redirect()`, `._require_csrf()`, `._required_user()`, `._send_asset()`, `._send_empty()`, `._send_html()`, `._send_json()`, `._send_security_headers()`, `._session_token()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 26`** (17 nodes): `.constructor()`, `.detach()`, `.detachPrimitive()`, `.ca()`, `.detached()`, `.detachPrimitive()`, `we`, `.applyOptions()`, `.constructor()`, `.detach()`, `.et()`, `.getSeries()`, `.ht()`, `.jp()`, `._M()`, `.mM()`, `.uM()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 32`** (13 nodes): `.sb()`, `.yb()`, `ir`, `._b()`, `.constructor()`, `.fb()`, `.lb()`, `.mb()`, `.ob()`, `.pb()`, `.clearMarkers()`, `.markers()`, `.markers()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 33`** (12 nodes): `di()`, `o`, `.constructor()`, `.hi()`, `.ht()`, `.i()`, `.m()`, `.p()`, `.st()`, `.$t()`, `.u()`, `.v()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 36`** (11 nodes): `it`, `.constructor()`, `.ih()`, `.Ns()`, `.st()`, `pt`, `.constructor()`, `.ih()`, `.st()`, `.kt()`, `.draw()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 37`** (11 nodes): `ct`, `.constructor()`, `.He()`, `.je()`, `.Ke()`, `.qe()`, `.Ue()`, `.Xe()`, `.Ye()`, `.Ze()`, `.Zi()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 38`** (11 nodes): `le`, `.constructor()`, `ne()`, `.attached()`, `.constructor()`, `.detached()`, `.hr()`, `.NM()`, `.paneViews()`, `.SM()`, `.updateAllViews()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 42`** (7 nodes): `be`, `.CM()`, `.constructor()`, `.dM()`, `.draw()`, `.et()`, `.ht()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 47`** (4 nodes): `f()`, `.constructor()`, `.ht()`, `.st()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 49`** (3 nodes): `main()`, `configure_telegram_webhook.py`, `telegram_call()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 50`** (3 nodes): `main()`, `configure_stripe_webhook.py`, `stripe_request()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 53`** (2 nodes): `Spread arbitrage research bot.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 54`** (2 nodes): `__init__.py`, `Utility helpers for spreadarb.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 55`** (2 nodes): `Narrow live-operation helpers.  The main live engine remains locked. Modules in`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 56`** (2 nodes): `Read-only decentralized exchange integrations.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 57`** (2 nodes): `Read-only API discovery for Telegram opportunity visibility.  The package is int`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `FastQuoteRefresher` connect `Community 4` to `Community 2`, `Community 23`, `Community 14`, `Community 22`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `WatchAsset` connect `Community 3` to `Community 10`, `Community 5`?**
  _High betweenness centrality (0.036) - this node is a cross-community bridge._
- **Why does `SpreadBoardServer` connect `Community 14` to `Community 2`, `Community 4`?**
