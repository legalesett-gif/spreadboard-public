...
## Corpus Check
- 110 files · ~214,352 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5405 nodes · 10381 edges · 86 communities detected
...
5. `vn()` - 54 edges
6. `ci` - 53 edges
7. `_float_or_none()` - 52 edges
8. `_connect()` - 51 edges
9. `fmt_signed_pct()` - 49 edges
10. `_reconciliation_summary()` - 49 edges
...
Cohesion: 0.01
Nodes (135): _(), a(), ae, ai, an(), ar(), as(), At (+127 more)
...
Cohesion: 0.01
Nodes (375): _erow(), _frow(), _funding_row(), _iso(), _live_row(), _mq(), _perp_dex_source_index(), _prow() (+367 more)

### Community 3 - "Community 3"
Cohesion: 0.02
Nodes (348): _daily_pct(), main(), A venue's per-interval rate as percent per day., A venue's per-interval rate as percent per day., _exchange(), main(), A loaded CCXT client for a venue id, or None if it has no adapter., active_class() (+340 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (273): _checks_from_payload(), empty(), ExecutorAttestation, ExecutorAttestationRegistry, load_executor_attestations(), Read-only executor/preflight attestations for API discovery rows., route_key(), BlacklistFilterResult (+265 more)
...
Cohesion: 0.02
Nodes (151): depth_weighted_price(), Small read-only order-book math helpers., main(), main(), _atomic_write(), _ccxt_current_funding(), _dex_chain_contract(), _expanded_token_rows() (+143 more)

### Community 7 - "Community 7"
Cohesion: 0.01
Nodes (155): _board_stream_rows(), _merge_history_rows(), Explain an empty farm tab instead of rendering a blank list.      Futures-DEX in, Merge proxy and exact samples without sacrificing full-window coverage., Merge proxy and exact samples without sacrificing full-window coverage., Explain an empty farm tab instead of rendering a blank list.      Futures-DEX in, Merge proxy and exact samples without sacrificing full-window coverage., Merge proxy and exact samples without sacrificing full-window coverage. (+147 more)
...
Cohesion: 0.04
Nodes (104): add_alert_rule(), add_funding_cashflow(), add_market_alert_rule(), add_saved_chart(), apply_billing_event(), _assert_customer_owner(), _billing_user_id(), bind_telegram_chat() (+96 more)

### Community 10 - "Community 10"
Cohesion: 0.02
Nodes (112): fmt_renewal_date(), Prepaid crypto checkout: pick a period, pay the exact amount, get access., Prepaid crypto checkout: pick a period, pay the exact amount, get access., Prepaid crypto checkout: pick a period, pay the exact amount, get access., Prepaid crypto checkout: pick a period, pay the exact amount, get access., Prepaid crypto checkout: pick a period, pay the exact amount, get access., Client logic: create an invoice, render a QR, poll until it settles.      Emits, Client logic: create an invoice, render a QR, poll until it settles.      Emits (+104 more)

### Community 11 - "Community 11"
Cohesion: 0.03
Nodes (83): _board_leg_key(), _board_legs(), BookWorker, _chunk_size_for(), _chunks(), _desired_legs(), _leg_key(), _levels() (+75 more)
...
Cohesion: 0.06
Nodes (8): cn(), dn(), gn, j, on, rn(), u(), Zn()

### Community 18 - "Community 18"
Cohesion: 0.04
Nodes (56): _address_from_topic(), get_cursor(), Watch Arbitrum One for confirmed stablecoin transfers to the receiving address., Scan one confirmed block range. Returns a summary of what was applied., Resident worker loop. Never raises; degrades to retrying on RPC failure., Resident worker loop. Never raises; degrades to retrying on RPC failure., Scan one confirmed block range. Returns a summary of what was applied., run_forever() (+48 more)

### Community 19 - "Community 19"
Cohesion: 0.03
Nodes (63): _balanced_route_candidates(), _candidate_is_publicly_rankable(), Use scarce depth checks on distinct assets before extra venue permutations., Keep one route class from consuming every matched-depth check., Keep obvious mirages from consuming the limited order-book budget., Use scarce depth checks on distinct assets before extra venue permutations., Keep one route class from consuming every matched-depth check., Use scarce depth checks on distinct assets before extra venue permutations. (+55 more)
...
Cohesion: 0.06
Nodes (45): _api_call(), config(), _configure_group(), _create_join_request_link(), _handle_callback(), _handle_group_query(), _handle_inline_query(), _handle_join_request() (+37 more)

### Community 22 - "Community 22"
Cohesion: 0.06
Nodes (31): BaseHTTPRequestHandler, Current prices for a lane, computed once however many streams want them., Current prices for a lane, computed once however many streams want them., Current prices for a lane, computed once however many streams want them., Current prices for a lane, computed once however many streams want them., Current prices for a lane, computed once however many streams want them., Current prices for a lane, computed once however many streams want them., Current prices for a lane, computed once however many streams want them. (+23 more)

### Community 23 - "Community 23"
Cohesion: 0.07
Nodes (5): ft, ir, lr, tr, yt
...
Cohesion: 0.06
Nodes (34): A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre, A member's own alerts, each against the value it is watching right now.      Cre (+26 more)

### Community 29 - "Community 29"
Cohesion: 0.11
Nodes (16): alertable_reopens(), detect_reopens(), _float(), format_alert(), _group_chat_id(), RailReopenWatcher, Alert when a shut transfer rail reopens while the spread is still wide.  A fat s, Poll the public rail cache and announce reopens that still pay. (+8 more)

### Community 30 - "Community 30"
Cohesion: 0.12
Nodes (24): _is_tradeable_perpetual(), _market_matches_type(), Only linear perpetuals belong on a funding board.      Kraken lists XRP three wa, Only linear perpetuals belong on a funding board.      Kraken lists XRP three wa, Only linear perpetuals belong on a funding board.      Kraken lists XRP three wa, Only linear perpetuals belong on a funding board.      Kraken lists XRP three wa, Only linear perpetuals belong on a funding board.      Kraken lists XRP three wa, Only linear perpetuals belong on a funding board.      Kraken lists XRP three wa (+16 more)
...
Cohesion: 0.14
Nodes (23): _align(), _atomic_json(), _cache_path(), _claim_warming(), evenly_sample(), _fetch_and_cache(), _fetch_leg(), _is_dex() (+15 more)
...
Cohesion: 0.1
Nodes (21): Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you, Realised 1d/7d/30d carry for a route, or an honest blank.      A rate tells you (+13 more)
...
Cohesion: 0.12
Nodes (17): _build_ourbit_exchange(), Ourbit has no ccxt adapter, but it is an MEXC white-label.      Its spot API is, Ourbit has no ccxt adapter, but it is an MEXC white-label.      Its spot API is, Ourbit has no ccxt adapter, but it is an MEXC white-label.      Its spot API is, Ourbit has no ccxt adapter, but it is an MEXC white-label.      Its spot API is, Ourbit has no ccxt adapter, but it is an MEXC white-label.      Its spot API is, Ourbit has no ccxt adapter, but it is an MEXC white-label.      Its spot API is, Ourbit has no ccxt adapter, but it is an MEXC white-label.      Its spot API is (+9 more)
...
## Knowledge Gaps
- **1996 isolated node(s):** `HTTP boundary for crypto checkout.  The subtle failure this guards against: memb`, `The tutorial must be reachable without an account -- it is a conversion page.`, `The crypto payment path, end to end.  The watcher is what makes a crypto payment`, `It was only started by server.py's CLI main(), which production does not run.`, `A cold cursor must not scan Arbitrum from block 0.` (+1991 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 43`** (17 nodes): `y`, `.constructor()`, `.cs()`, `.ds()`, `.es()`, `.fs()`, `.Gn()`, `.gs()`, `.hs()`, `.ls()`, `.ns()`, `._s()`, `.ss()`, `.st()`, `.ts()`, `.us()`, `.ws()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 46`** (16 nodes): `js()`, `.cacheKey()`, `.constructor()`, `.convertHorzItemToInternal()`, `.createConverterToInternalObj()`, `.fillWeightsForPoints()`, `.formatHorzItem()`, `.formatTickmark()`, `.key()`, `.maxTickMarkWeight()`, `.options()`, `.preprocessData()`, `.setOptions()`, `.updateFormatter()`, `.zg()`, `.eg()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 56`** (13 nodes): `.width()`, `.jp()`, `we`, `.applyOptions()`, `.constructor()`, `.et()`, `.getSeries()`, `.ht()`, `.jp()`, `._M()`, `.mM()`, `.oM()`, `.uM()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 59`** (12 nodes): `di()`, `o`, `.constructor()`, `.hi()`, `.ht()`, `.i()`, `.m()`, `.p()`, `.st()`, `.$t()`, `.u()`, `.v()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 64`** (11 nodes): `ct`, `.constructor()`, `.He()`, `.je()`, `.Ke()`, `.qe()`, `.Ue()`, `.Xe()`, `.Ye()`, `.Ze()`, `.Zi()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 69`** (8 nodes): `.paneViews()`, `.updateAllViews()`, `rt`, `.constructor()`, `.Fn()`, `.oh()`, `.On()`, `.Ys()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 70`** (7 nodes): `be`, `.CM()`, `.constructor()`, `.dM()`, `.draw()`, `.et()`, `.ht()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 72`** (6 nodes): `dt`, `.constructor()`, `.Ke()`, `.Qe()`, `.tr()`, `.Xe()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 73`** (5 nodes): `test_route_index.py`, `Opening a chart by route must not rebuild the whole board.  _find_canonical_rout`, `test_a_catalogue_route_short_circuits()`, `test_a_new_snapshot_rebuilds_the_index()`, `test_the_board_is_built_once_and_then_indexed()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 77`** (4 nodes): `f()`, `.constructor()`, `.ht()`, `.st()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 78`** (4 nodes): `it`, `.constructor()`, `.ih()`, `.Ns()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 80`** (3 nodes): `main()`, `configure_telegram_webhook.py`, `telegram_call()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 81`** (3 nodes): `main()`, `configure_stripe_webhook.py`, `stripe_request()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 82`** (3 nodes): `qt`, `.constructor()`, `.lr()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 87`** (2 nodes): `Spread arbitrage research bot.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 88`** (2 nodes): `__init__.py`, `Utility helpers for spreadarb.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (2 nodes): `Narrow live-operation helpers.  The main live engine remains locked. Modules in`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 90`** (2 nodes): `Read-only decentralized exchange integrations.`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 91`** (2 nodes): `Read-only API discovery for Telegram opportunity visibility.  The package is int`, `__init__.py`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `q()` connect `Community 0` to `Community 56`, `Community 2`?**
  _High betweenness centrality (0.285) - this node is a cross-community bridge._
- **Why does `_Book` connect `Community 4` to `Community 1`, `Community 2`, `Community 5`, `Community 6`?**
  _High betweenness centrality (0.235) - this node is a cross-community bridge._
- **Why does `FastQuoteRefresher` connect `Community 6` to `Community 3`, `Community 4`, `Community 5`, `Community 39`, `Community 22`, `Community 27`?**
